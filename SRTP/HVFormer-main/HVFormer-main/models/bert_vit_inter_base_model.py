...
class CLIPVisionEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values, aux_embeddings=None, rcnn_embeddings=None):
        ...
        elif aux_embeddings is not None:
            batch_size = aux_embeddings.shape[0]
        elif rcnn_embeddings is not None:
            batch_size = rcnn_embeddings.shape[0]
        else:
            raise ValueError("At least one visual input tensor is required.")
        ...
        if pixel_values is not None:
            ...
            patch_embeds = patch_embeds + self.position_embedding(self.position_ids[:, :patch_embeds.size(1)])
...
class Vision2TextLayerWiseAgg(nn.Module):
    ...
    def forward(
        ...
        if getattr(self.args, "disable_cross_modal", False):
            outputs = (text_hidden_states,)
            if output_attentions:
                outputs += (None,)
            return outputs
        ...
        if getattr(self.args, "disable_moe_fusion", False):
            cross_modal_hidden_state = torch.stack(list(layer_wise_visual_hidden_states), dim=0).mean(dim=0)
        else:
            cross_modal_hidden_state = self.moe_layer_wise_agg(
                text_hidden_states=text_hidden_states,
                layer_wise_visual_hidden_states=layer_wise_visual_hidden_states,
                attention_mask=attention_mask,
            )
        outputs = (cross_modal_hidden_state,)
        if output_attentions and all_cross_modal_attentions is not None:
